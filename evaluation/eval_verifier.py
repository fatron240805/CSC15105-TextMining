#!/usr/bin/env python3
"""Đánh giá vai trò #3 — LLM-verifier khử dương-tính-giả. KHÁCH QUAN (nhãn từ gold PAN).

Quy trình:
  align susp vs nguồn GOLD -> span dự đoán -> gán nhãn TP/FP theo giao với gold ->
  verifier quyết định GIỮ/BỎ -> đo:
    - ma trận nhầm {giữ,bỏ} × {TP,FP}, FP-reduction, TP-retention
...
def main():
    ...
    for su, golds in gold_by.items():
        ...
        spans = align_pair(stext, rtext, TH, TH, TH3, 4)
        if not spans:
            continue
        for go, gl in golds:
            gold_spans.append(Span(su, go, gl))
        for s, l, ss, sl in spans:
            if budget <= 0:
                break
            budget -= 1
            ...
            rows.append({"doc": su, "tp": tp, "keep": keep, "conf": v.get("confidence")})
            print(f"  {su[:20]} {'TP' if tp else 'FP'} -> {'GIỮ' if keep else 'BỎ '} "
                  f"(conf {v.get('confidence')})", flush=True)

    # ma trận nhầm
    n_tp = sum(1 for r in rows if r["tp"]); n_fp = len(rows) - n_tp
    keep_tp = sum(1 for r in rows if r["tp"] and r["keep"])
    keep_fp = sum(1 for r in rows if not r["tp"] and r["keep"])
    fp_removed = n_fp - keep_fp
    b = plagdet_score(gold_spans, pred_before)
    a = plagdet_score(gold_spans, pred_after)

    summary = {
        "n_spans": len(rows), "n_TP": n_tp, "n_FP": n_fp, "n_fail": fails,
    ...
    print("\n===== EVAL #3 — VERIFIER KHỬ DƯƠNG-TÍNH-GIẢ =====")
    print(f"span: {len(rows)} ({n_tp} TP, {n_fp} FP) · {fails} lỗi")
    ...
    print(f"  HEADLINE PlagDet: {b.plagdet:.3f} -> {a.plagdet:.3f}  |  "
